...
class ViewFactory(object):
    model_mesh_map = {
        AsteroidModel: "_spheroid",
        ShieldModel: "_hexagon_fence"
    }
    ...
    def _mesh_for_model(self, model):
        func_name = self.model_mesh_map.get(model.__class__, "_mesh_by_name")
    ...
    def _spheroid(self, model):
        faces = []
        point_seven = (0.7, 0.7, 0.7)
        zeroes = (0, 0, 0)
        material = OpenGLTexturedMaterial(diffuse=point_seven, emissive=zeroes, ambient=zeroes, shininess=0.0,
                                          specular=zeroes, alpha=1.0, name="Rock Surface",
                                          texture_file_name="ROCKS002.TGA")
    ...
    def _hexagon_fence(self, model):
        faces = []
        baby_blue = (.54, .81, .94)
        zeroes = (0, 0, 0)
        material = OpenGLMaterial(diffuse=zeroes, emissive=baby_blue, ambient=zeroes, shininess=0.0,
                                  specular=zeroes, alpha=.5, name="Shield")
